...
public class BoardWriteOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 자료실 폼 페이지에서 넘어온 데이터들을 DB(upload)에 저장하는 비지니스 로직
				...
				// 2. 첨부 파일 크기 지정(1024 -> 1KB) 
				int fileSize = 20 * 1024 * 1024; // 10MB (보통 10~20MB로 제한)
				...
				String board_cont = multi.getParameter("board_cont").trim();
				  ...
				  // 우선은 첨부파일의 이름을 알아야 함 // getName() 메서드를 이용하면 첨부파일의 이름을 알 수 있음 String
				  fileName = upload_file.getName(); System.out.println("첨부파일 이름 >>> " +
				  fileName);
				  ...
				  // ....../fileUpload/2023-03-28 String homedir =
				  saveFolder+"/"+year+"-"+month+"-"+day;
				  
				  // 날짜 폴더를 만들어 보자 File path1 = new File(homedir);
				  
				  if(!path1.exists()) { // 폴더가 존재하지 않는 경우 path1.mkdir(); //실제 폴더를 만들어주는 메서드 }
				  
				  // 파일을 만들어 보자 ==> 예) 홍길동_파일명 String reFileName = upload_writer+"_"+fileName;
				  
				  // 파일 이름 변경 upload_file.renameTo(new File(homedir+"/"+reFileName));
				  
				  // 실제로 DB에 저장되는 파일 이름 // "/2023-03-28/홍길동_파일명)" 으로 저장할 예정 String fileDBName =
